...
public class ParametersTests
{
    ...
    [InlineData(" ")]
    [InlineData(null)]
    public async Task DeleteSavedMovie_UserIdHasInvalidValues_ReturnsBadRequest(string userId)
    {
        //Arrange
        ...
        //Act
        var response = await _sut.DeleteSavedMovie(request, userId, 1, _logger);
    ...
    [InlineData(0)]
    [InlineData(-1)]
    public async Task DeleteSavedMovie_MovieIdHasInvalidValues_ReturnsBadRequest(int movieId)
    {
        //Arrange
        var request = new DefaultHttpRequest(new DefaultHttpContext())
        {
        ...
        //Act
        var response = await _sut.DeleteSavedMovie(request, "userId", movieId, _logger);
        var result = (ContentResult)response;
